...
int main(int argc, char *argv[])
{
    ...
    auto channel = sm->getChannel(FLAGS_speech_service);
    if (!channel) {
    ...
    // 4. 发起EchoRpc调用
    liren::SpeechService_Stub stub(channel.get());
    liren::SpeechRecognitionReq req;
    req.set_speech_content(file_content);
    req.set_request_id("111111");

    brpc::Controller *cntl = new brpc::Controller();
    liren::SpeechRecognitionRsp *rsp = new liren::SpeechRecognitionRsp();
